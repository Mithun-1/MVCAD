...
void Viewport::paintEvent(QPaintEvent*){
    QPainter p(this);p.fillRect(rect(),sketch_>=0?QColor("#fcfdff"):QColor("#fff"));p.setRenderHint(QPainter::Antialiasing,true);faces_.clear();
    ...
    auto triangles=[&](const std::vector<Triangle>& mesh,bool cad){for(const auto& t:mesh){auto a=rotate(t.a),b=rotate(t.b),c=rotate(t.c);auto normal=QVector3D::crossProduct(b-a,c-a).normalized();float shade=.4f+.6f*std::abs(QVector3D::dotProduct(normal,light));QColor base=cad?QColor("#91a9c2"):((t.branch>=0&&t.branch==selected_)?QColor("#00b7cc"):(t.branch<0&&std::any_of(vessels_.junctions.begin(),vessels_.junctions.end(),[&](const auto& j){return j.index==-t.branch-1&&j.state==VesselBuildState::Provisional;})?QColor("#b781cc"):QColor("#c83f55")));QColor color;color.setRgbF(base.redF()*shade,base.greenF()*shade,base.blueF()*shade);if(sketch_>=0)color.setAlpha(60);faces_.push_back({{project(t.a),project(t.b),project(t.c)},(a.z()+b.z()+c.z())/3,color,cad?-1000:t.branch});}};
    triangles(cad_.triangles,true);triangles(preview_.triangles,false);std::sort(faces_.begin(),faces_.end(),[](const Face&a,const Face&b){return a.depth<b.depth;});p.setRenderHint(QPainter::Antialiasing,false);for(const auto& f:faces_){p.setPen(QPen(f.color,.5));p.setBrush(f.color);p.drawPolygon(f.polygon);}p.setRenderHint(QPainter::Antialiasing,true);p.setBrush(Qt::NoBrush);
...
void Viewport::mouseReleaseEvent(QMouseEvent* e){
    ...
    int selected=-1;for(auto it=faces_.rbegin();it!=faces_.rend();++it)if(it->polygon.containsPoint(e->position(),Qt::OddEvenFill)){if(it->branch>=0)selected=it->branch;break;}
